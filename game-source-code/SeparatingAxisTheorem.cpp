...
bool SeparatingAxisTheorem::isOverlapping(HitBox& obj_1, HitBox& obj_2)
{
	if(!axis_.empty())
		clearAll();
...
two_floats SeparatingAxisTheorem::normalizeAxis(two_floats& axis)
{
...
void SeparatingAxisTheorem::makeAxes()
{
...
float SeparatingAxisTheorem::getDotProduct(const two_floats& pointA, const two_floats& pointB)
{
	auto dot_product = (std::get<0>(pointA) * std::get<0>(pointB)) + (std::get<1>(pointA) * std::get<1>(pointB));
	return dot_product;
}

two_floats SeparatingAxisTheorem::projection(const two_floats& axis,vector<two_floats>& verticies)
{
...
bool SeparatingAxisTheorem::checkProjectionOverLap()
{
